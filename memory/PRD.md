...
- **Native browser/phone back button** verified to restore exact scroll position identically to the on-page back link (sessionStorage restore runs on Site remount regardless of POP vs Link navigation).
- **Mobile demo cards**: whole card is a comfortable tap target (bigger padding + min-height 190px, `:active` press feedback), "Megnézem →" CTA as accent pill on ≤760px.
- **Per-agent "Ajánlatot kérek" button** (2026-07): all 12 agent cards (`Agents.jsx` TiltCard) now show a compact gradient quote button — same style/text as the website-package CTA (`t.pricing.cta`) but much smaller (`.card-quote` in `index.css`: 9px/18px padding, 12px mono). mailto href includes the agent name (`{subjectPrefix} – {agent.title}`). Placed below card-meta, above try/sim buttons. Verified: 12 buttons, correct hrefs, readable on desktop + 380px mobile (0 overflow).
